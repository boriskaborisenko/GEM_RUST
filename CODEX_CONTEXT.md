...
- Optional LLM forecasting is configured under `llm` in `config.json`, for example `{ "enabled": true, "model": "gemini-3.5-flash", "location": "global" }`, and requires Vertex service-account credentials at `GEM_RUST/llm.json` (`.gitignore` excludes it). `src/llm.rs` runs a real startup `hello` health check before enabling LLM for the session, then sends exactly one pre-window prompt per `window_number`; for `location: "global"` it uses the global Vertex endpoint `aiplatform.googleapis.com` with resource path `locations/global`. The prompt includes rolling context from the last 10 closed windows in the same process: avg/median PnL, max drawdown, entry-side accuracy, LLM accuracy, runner redeem rate, hedge cost/rescue PnL, tail liquidation value, adverse slippage at 0.01/0.02, winner counts, and compact per-window rows. D1 now uses a qualified LLM forecast as a soft pre-start side-selection factor after a real PTB/fair directional signal, but before plain momentum/bootstrap. The thresholds are LLM confidence, not contract price: `>=0.60` for 15m and `>=0.68` for faster windows, with `medium|strong` strength and the actual BUY ask still bounded by `preStartEntry.minSideAsk..maxSideAsk` (`0.42..0.58` in current config). The terminal shows `LLM-forecast: enabled/disabled`, model, location, right/wrong counts, and accuracy. `entry_events.csv` records LLM side/confidence next to the actual entry side, and `llm_forecasts.csv` records forecast rows plus result rows with `result_winner` and `result_correct` so LLM directional hit-rate can be audited independently of PnL.
