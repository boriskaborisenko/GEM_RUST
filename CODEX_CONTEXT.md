...
- Optional LLM forecasting is configured under `llm` in `config.json`, for example `{ "enabled": true, "model": "gemini-3.5-flash" }`, and requires Vertex service-account credentials at `GEM_RUST/llm.json` (`.gitignore` excludes it). `src/llm.rs` sends exactly one pre-window prompt per `window_number`. For now the forecast is research-only: D1 ignores it for buying decisions. The terminal shows `LLM-forecast: enabled/disabled`, model, right/wrong counts, and accuracy. `entry_events.csv` records LLM side/confidence next to the actual entry side, and `llm_forecasts.csv` records forecast rows plus result rows with `result_winner` and `result_correct` so LLM directional hit-rate can be audited independently of PnL.
